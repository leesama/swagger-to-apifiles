...
const setTemplatesToSchema = (schema, serviceMap) => {
    ...
    Object.keys(schema.paths).forEach((requestUrl) => {
        ...
        const methodName = camelCase(firstUrlItem) + pascalCase(secondUrlItem.replace(".json", ""));
    ...
    '${requestUrl}',
    {
      ...
      params
        ...
        if (path.post) {
            ...
            const requestType = path.post?.consumes[0] === "application/json"
                ? "json"
                : "form";
            const template = `
// ${description}
  ...
  return request<${typePrefix}ReqRes>(
    '${requestUrl}',
    {
      ...
      requestType:'${requestType}'
        ...
        currentService.filePath = join(basePath, `${currentService.controllerShortName}.ts`);
...
//# sourceMappingURL=setTemplatesToSchema.js.map